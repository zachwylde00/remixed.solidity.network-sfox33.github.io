...
{
	var array = [this.array[0] - v.array[0], this.array[1] - v.array[1]]
...
{
    var alpha = pointLineDist(p2,p1,p)/pointLineDist(p2,p1,p0);	//Calculates the alpha cooefficient in barycentric coordinates
	var beta = pointLineDist(p0,p2,p)/pointLineDist(p0,p2,p1);		//Calculates the beta cooefficient in barycentric coordinates
	var gamma = pointLineDist(p0,p1,p)/pointLineDist(p0,p1,p2);	//Calculates the gamme cooefficient in barycentric coordinates
...
{
    var numerator = new Mat2();
	numerator.set(0, 0, p1.x-p0.x);	//Sets element M_00
	numerator.set(0, 1, p1.y-p0.y);	//Sets element M_10
	numerator.set(1, 0, p0.x-p.x);		//Sets element M_01
	numerator.set(1, 1, p0.y-p.y);		//Sets element M_11
	var determinant = numerator.det();	//Calculates the determinant of the numerator
	var denominator = new Vec2([p1.x - p0.x, p1.y - p0.y]);
	var magnitude = denominator.mag();	//Determines the magnitude of the denominator
